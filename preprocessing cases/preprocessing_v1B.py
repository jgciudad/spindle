...
def get_eeg_spectrogram(waveform, fs=128, stft_size=256, stft_stride=16, lowcut=0.5, highcut=24):
    # STFT
    Z = tf.signal.stft(
        waveform, frame_length=stft_size, frame_step=stft_stride)
...
def get_eeg_spectrogram_no_batch(waveform, fs=128, stft_size=256, stft_stride=16, lowcut=0.5, highcut=24):
    ...
    # Standardize
    y_mean = tf.math.reduce_mean(y, axis=0)
    y_std = tf.math.reduce_std(y, axis=0)

    y = (y - tf.expand_dims(y_mean, axis=0)) / tf.expand_dims(y_std, axis=0)
...
def load_and_preprocess_data_2(file_path):
    ...
    #     get_emg_spectrogram_no_batch(b)
...
def visualize_dataset(dataset, same_scale=True):
    ...
    for i in range(3): # for the 3 signals (EEG_1, EEG_2, EMG)
            ...
            for b, c in dataset:
                if counter < 6:
                    ...
                    if len(idx) != 0:# and len(idx)>20:
                        ...
                        img = ax[j, counter].imshow(tf.transpose(spect), cmap='jet')#, vmin=-7, vmax=2.5)
                        ...
                        ax[j, counter].set_yticklabels(['0,5', '12', '24'])
        ...
        # plt.show()
        fig.savefig(suptitle + '_preprocessing_v1')
