...
def preprocess_EEG(signal, fs=128, stft_size=256, stft_stride=16, lowcut=0.5, highcut=24, visualize=True):
    ...
    if visualize:
        ...
        img = cax.imshow(np.abs(Z), cmap='jet', aspect='auto')
        ...
        #            ymin=cax.get_ylim()[0], ymax=cax.get_ylim()[1], color='k')
        fig.colorbar(img, cax=ax[1, 1], ticks=[np.min(np.abs(Z)), np.max(np.abs(Z))])
    ...
    if visualize:
        ...
        # cax.vlines(x=cax.get_xticks()[1:-1],
        #            ymin=cax.get_ylim()[0], ymax=cax.get_ylim()[1], color='k')
        fig.colorbar(img, cax=ax[2, 1], ticks=[np.min(np.abs(Z)), np.max(np.abs(Z))])
    ...
    if visualize:
        ...
        img = cax.imshow(y, cmap='jet', aspect='auto')
        cax.set_title('PSD')
        cax.invert_yaxis()
        # cax.set_xlabel('Time (s)')
        cax.set_ylabel('Frequency (Hz.)')
        ...
        # cax.vlines(x=cax.get_xticks()[1:-1],
        #            ymin=cax.get_ylim()[0], ymax=cax.get_ylim()[1], color='k')
        fig.colorbar(img, cax=ax[3, 1], ticks=[np.min(y), np.max(y)])
    ...
    if visualize:
        ...
        # cax.vlines(x=cax.get_xticks()[1:-1],
        #            ymin=cax.get_ylim()[0], ymax=cax.get_ylim()[1], color='k')
        fig.colorbar(img, cax=ax[4, 1], ticks=[np.min(y), np.max(y)])
    ...
    if visualize:
        ...
        # cax.vlines(x=cax.get_xticks()[1:-1],
        #            ymin=cax.get_ylim()[0], ymax=cax.get_ylim()[1], color='k')
        fig.colorbar(img, cax=ax[5, 1], ticks=[np.min(y), np.max(y)])
        plt.show()
...
def load_and_preprocess_data_1(file_path):
    ...
    input_dataset = tf.data.Dataset.from_tensor_slices(wrt)
...
def visualize_dataset(dataset, same_scale=True):
    ...
    for i in range(3): # for the 3 signals (EEG_1, EEG_2, EMG)
        ...
        # plt.show()
        fig.savefig('v1A_' + suptitle + '_preprocessing')
